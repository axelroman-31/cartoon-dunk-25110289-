...
// ─────────────────────────────────────────
//  Datos de equipos  -  sprites articulados
// ─────────────────────────────────────────
static DatosEquipo equipoCartoon() {
    ...
    d.fWalk[1]=7; d.fRun[1]=7; d.fDribble[1]=4; d.fShoot[1]=3; d.fDunk[1]=3;
    d.altoPx[1] = 68.f;

    // BUGS: sheet 387x518 | 7 cols x 5 rows | frame 55x103
    d.sheetCols[2]=7; d.sheetRows[2]=5;
    d.fWalk[2]=7; d.fRun[2]=6; d.fDribble[2]=5; d.fShoot[2]=4; d.fDunk[2]=4;
    d.altoPx[2] = 74.f;
...
static DatosEquipo equipoRival() {
    ...
    d.spritesStatic[1] = "assets/imagenes/mistico  jugador.png";
...
public:
    ...
    PantallaIntro(sf::Font& f,bool fok):F(f),Fok(fok){
        ...
        if(okFondo){
            ...
            float sx=(float)W_ANCHO/texFondo.getSize().x,sy=(float)W_ALTO/texFondo.getSize().y;
            sprFondo.setScale(std::max(sx,sy),std::max(sx,sy));
            sprFondo.setColor(sf::Color(255,255,255,0));
        }
        auto ss=[](sf::Sprite& s,sf::Texture& t,float h,float x,float y){
            s.setTexture(t); float sc=h/t.getSize().y; s.setScale(sc,sc);
            s.setPosition(x,y); s.setColor(sf::Color(255,255,255,0));
        };
        if(okGA)  ss(sprGokuArt,  texGokuArt,  200.f,30.f,(float)W_ALTO-210.f);
        if(okPA)  ss(sprPocoyoArt,texPocoyoArt,120.f,210.f,(float)W_ALTO-135.f);
        if(okMA){ sprMisticoArt.setTexture(texMisticoArt);
            float sc=200.f/texMisticoArt.getSize().y; sprMisticoArt.setScale(sc,sc);
            float pw=texMisticoArt.getSize().x*sc;
            sprMisticoArt.setPosition((float)W_ANCHO-pw-30.f,(float)W_ALTO-210.f);
            sprMisticoArt.setColor(sf::Color(255,255,255,0)); }
        if(okPB){ sprPixelBugs.setTexture(texPixelBugs);
            float sc=95.f/texPixelBugs.getSize().y; sprPixelBugs.setScale(sc,sc);
            float pw=texPixelBugs.getSize().x*sc;
            sprPixelBugs.setPosition((float)W_ANCHO-pw-200.f,(float)W_ALTO-110.f);
            sprPixelBugs.setColor(sf::Color(255,255,255,0)); }
        for(int i=0;i<60;i++){
            Part p; p.x=(float)(rand()%W_ANCHO); p.y=(float)(rand()%W_ALTO);
            p.vel=18.f+(rand()%40); p.tam=2.f+(rand()%4); p.alfa=0.f;
            int c=rand()%4;
            if(c==0)p.col=sf::Color(255,200,20); else if(c==1)p.col=sf::Color(80,200,255);
            else if(c==2)p.col=sf::Color(255,60,120); else p.col=sf::Color(120,255,100);
            partes.push_back(p);
        }
    }
    void actualizar(float dt){
        tTotal+=dt; ondaY=std::sin(tTotal*1.8f)*6.f; parpadeo+=dt*2.8f;
        ...
        if(okGA){  sprGokuArt.setPosition(30.f,(float)W_ALTO-210.f+ondaY*0.6f);   sprGokuArt.setColor({255,255,255,ap}); }
        if(okMA){  float pw=texMisticoArt.getSize().x*sprMisticoArt.getScale().x;
                   sprMisticoArt.setPosition((float)W_ANCHO-pw-30.f,(float)W_ALTO-210.f-ondaY*0.6f);
                   sprMisticoArt.setColor({255,255,255,ap}); }
        if(okPA){  sprPocoyoArt.setPosition(210.f,(float)W_ALTO-135.f+ondaY*0.4f);  sprPocoyoArt.setColor({255,255,255,ap}); }
        if(okPB){  float pw=texPixelBugs.getSize().x*sprPixelBugs.getScale().x;
                   sprPixelBugs.setPosition((float)W_ANCHO-pw-200.f,(float)W_ALTO-110.f-ondaY*0.4f);
                   sprPixelBugs.setColor({255,255,255,ap}); }
        for(auto& p:partes){
            p.y-=p.vel*dt; p.alfa=std::min(p.alfa+dt*120.f,180.f);
            if(p.y<-10.f){p.y=(float)W_ALTO+5.f;p.x=(float)(rand()%W_ANCHO);p.alfa=0.f;}
        }
    }
    void dibujar(sf::RenderWindow& w){
        sf::RectangleShape bg({(float)W_ANCHO,(float)W_ALTO}); bg.setFillColor(sf::Color(8,4,18)); w.draw(bg);
        if(okFondo) w.draw(sprFondo);
        sf::RectangleShape ov({(float)W_ANCHO,(float)W_ALTO}); ov.setFillColor(sf::Color(0,0,0,110)); w.draw(ov);
        for(auto& p:partes){
            sf::CircleShape c(p.tam); c.setPosition(p.x,p.y);
            sf::Color col=p.col; col.a=(sf::Uint8)p.alfa; c.setFillColor(col); w.draw(c);
        }
        ...
        if(escLogo>0.01f){
            ...
            if(escLogo>=0.9f){
                float lw=380.f*escLogo; sf::RectangleShape ln({lw,2.f});
                ln.setFillColor(sf::Color(255,180,0,160)); ln.setOrigin(lw/2.f,1.f);
                ln.setPosition((float)W_ANCHO/2.f,178.f); w.draw(ln);
        ...
        if(escLogo>=0.9f)
            dtxt(w,"WASD:Mover  J:Tiro  K:Pase  J+K:Dunk  L:Finta/Robo  Tab:Cambiar",11,sf::Color(110,110,120,150),(float)W_ANCHO/2.f,(float)W_ALTO-20.f,1.f);
    }
private:
    void dtxt(sf::RenderWindow& w,const std::string& s,int sz,sf::Color c,float x,float y,float esc){
        sf::Text t; if(Fok)t.setFont(F);
        t.setString(s); t.setCharacterSize(sz); t.setFillColor(c);
        t.setOutlineThickness(2.5f); t.setOutlineColor(sf::Color(0,0,0,(sf::Uint8)(c.a*0.7f)));
        sf::FloatRect b=t.getLocalBounds(); t.setOrigin(b.width/2.f,b.height/2.f);
        t.setScale(esc,esc); t.setPosition(x,y); w.draw(t);
    }
};
...
public:
    ...
    MenuPrincipal(sf::Font& f,bool fok):F(f),Fok(fok){
        ...
        auto ss=[](sf::Sprite& s,sf::Texture& tx,float h,float x,float y){
            s.setTexture(tx); float sc=h/tx.getSize().y; s.setScale(sc,sc); s.setPosition(x,y);
        };
        if(okG) ss(sprG,texG,250.f,30.f,(float)W_ALTO-260.f);
        if(okP) ss(sprP,texP,130.f,255.f,(float)W_ALTO-145.f);
        if(okB){ ss(sprB,texB,170.f,0.f,0.f);
            float pw=texB.getSize().x*sprB.getScale().x; sprB.setPosition((float)W_ANCHO-pw-30.f,(float)W_ALTO-180.f); }
        if(okM){ ss(sprM,texM,195.f,0.f,0.f);
            float pw=texM.getSize().x*sprM.getScale().x; sprM.setPosition((float)W_ANCHO-pw-215.f,(float)W_ALTO-205.f); }
        for(int i=0;i<18;i++){
            Bola b; b.x=(float)(rand()%W_ANCHO); b.y=(float)(rand()%W_ALTO);
            b.vel=12.f+(rand()%25); b.r=4.f+(rand()%8); b.alfa=0.f; bolas.push_back(b);
        }
    }
    void resetear(){alfaE=0.f;sel=0;sub=Sub::NINGUNA;}
    void actualizar(float dt){
        t+=dt; ondaY=std::sin(t*1.4f)*7.f; alfaE=std::min(alfaE+dt*280.f,255.f);
        if(okG) sprG.setPosition(30.f,(float)W_ALTO-260.f+ondaY*0.7f);
        if(okP) sprP.setPosition(255.f,(float)W_ALTO-145.f+ondaY);
        if(okB){float pw=texB.getSize().x*sprB.getScale().x; sprB.setPosition((float)W_ANCHO-pw-30.f,(float)W_ALTO-180.f-ondaY*0.5f);}
        if(okM){float pw=texM.getSize().x*sprM.getScale().x; sprM.setPosition((float)W_ANCHO-pw-215.f,(float)W_ALTO-205.f+ondaY*0.8f);}
        for(auto& b:bolas){b.y-=b.vel*dt;b.alfa=std::min(b.alfa+dt*80.f,70.f);
            if(b.y<-20.f){b.y=(float)W_ALTO+10.f;b.x=(float)(rand()%W_ANCHO);b.alfa=0.f;}}
    }
    bool procesarTecla(sf::Keyboard::Key k){
        if(sub!=Sub::NINGUNA){
            if(k==sf::Keyboard::Escape||k==sf::Keyboard::Return||k==sf::Keyboard::BackSpace) sub=Sub::NINGUNA;
            return false;
        }
    ...
    bool quiereSalir(sf::Keyboard::Key k){return sel==3&&sub==Sub::NINGUNA&&(k==sf::Keyboard::Return||k==sf::Keyboard::Space);}
    void dibujar(sf::RenderWindow& w){
        sf::RectangleShape bg({(float)W_ANCHO,(float)W_ALTO}); bg.setFillColor(sf::Color(6,4,16)); w.draw(bg);
        if(okFondo) w.draw(sprF);
        sf::RectangleShape piz({420.f,(float)W_ALTO}); piz.setFillColor(sf::Color(0,0,0,145)); w.draw(piz);
        for(auto& b:bolas){
            sf::CircleShape c(b.r); c.setPosition(b.x-b.r,b.y-b.r);
            c.setFillColor(sf::Color(255,140,20,(sf::Uint8)b.alfa)); w.draw(c);
        }
        if(okG) w.draw(sprG); if(okP) w.draw(sprP); if(okB) w.draw(sprB); if(okM) w.draw(sprM);
        ...
        sf::RectangleShape sep({330.f,2.f}); sep.setOrigin(165.f,1.f); sep.setPosition(210.f,140.f);
        sep.setFillColor(sf::Color(255,180,0,(sf::Uint8)(ae*0.5f))); w.draw(sep);
        float yB=182.f,paso=68.f;
        for(int i=0;i<4;i++){
            bool s=(i==sel); float yO=yB+i*paso;
            if(s){
                float pu=std::sin(t*4.f)*0.04f+1.f;
                sf::RectangleShape bx({310.f*pu,50.f}); bx.setOrigin(310.f*pu/2.f,25.f); bx.setPosition(210.f,yO+8.f);
                sf::Color cf=ops[i].cs; cf.a=(sf::Uint8)(ae*0.18f); bx.setFillColor(cf);
                bx.setOutlineThickness(2.f); sf::Color cb=ops[i].cs; cb.a=(sf::Uint8)(ae*0.7f); bx.setOutlineColor(cb); w.draw(bx);
            ...
            sf::Color col=s?ops[i].cs:ops[i].cn; col.a=ae;
...
private:
    void tx(sf::RenderWindow& w,const std::string& s,int sz,sf::Color c,float x,float y){
        sf::Text t; if(Fok)t.setFont(F);
        t.setString(s); t.setCharacterSize(sz); t.setFillColor(c);
        t.setOutlineThickness(2.f); t.setOutlineColor(sf::Color(0,0,0,(sf::Uint8)(c.a*0.6f)));
        sf::FloatRect b=t.getLocalBounds(); t.setOrigin(b.width/2.f,b.height/2.f);
    ...
    void panel(sf::RenderWindow& w,sf::Color borde){
        ...
        p.setPosition(60.f,40.f); p.setFillColor(sf::Color(8,5,20,232));
        p.setOutlineThickness(3.f); p.setOutlineColor(borde); w.draw(p);
    }
    void dibujarCtrl(sf::RenderWindow& w){
        ...
        struct L{std::string k,a;};
        std::vector<std::pair<std::string,std::vector<L>>> sec={
            {"MOVIMIENTO",{{"W/A/S/D","Mover"},{"LShift","Sprint"},{"Tab","Cambiar jugador"}}},
            {"OFENSIVA",{{"J","Tiro normal (2 o 3 pts)"},{"K","Pasar al companero libre"},
                {"J+K","Saltar para dunk"},{"J (en aire)","Ejecutar dunk"},
                {"L","Finta / amago"},{"Super+J","Super Shot"}}},
            {"DEFENSIVA",{{"J","Bloquear tiro / Empujar"},{"K","Robo de balon"},
                {"L","Empuje directo"},{"J+K (sin bal)","Alley-oop receptor"}}},
            {"SUPER METER",{{"Llena con:","Encestes pases robos bloqueos alley-oops"},
                {"Al llenarse:","Siguiente tiro = Super Shot imparable"}}}
        };
        float y=120.f;
        for(auto& se:sec){
            sf::Text ts; if(Fok)ts.setFont(F); ts.setString(se.first); ts.setCharacterSize(15);
            ts.setFillColor(sf::Color(255,200,50)); ts.setPosition(140.f,y); w.draw(ts); y+=22.f;
            for(auto& l:se.second){
                sf::Text tk,ta; if(Fok){tk.setFont(F);ta.setFont(F);}
                tk.setString(l.k); tk.setCharacterSize(13); tk.setFillColor(sf::Color(150,220,255)); tk.setPosition(155.f,y); w.draw(tk);
                ta.setString(l.a); ta.setCharacterSize(13); ta.setFillColor(sf::Color(210,210,210)); ta.setPosition(520.f,y); w.draw(ta);
                y+=18.f;
            } y+=8.f;
        }
    ...
    void dibujarCred(sf::RenderWindow& w){
        ...
        std::vector<C> cs={
            {"JUEGO","Cartoon Dunk"},{"INSPIRADO EN","Street Slam / Street Hoop (Data East 1994)"},
            {"MOTOR","SFML 2.6 | C++17"},{"",""},
            {"PERSONAJES","Goku | Pocoyo | Bugs Bunny | Mistico"},
            {"MUSICA","Gang$tazz.ogg"},{"FUENTE","texto.ttf"},{"",""},
            {"MECANICAS","Tiro 2/3 pts | Dunk | Super Shot | Alley-Oop | Finta | Bloqueo"},
        ...
        for(auto& c:cs){
            ...
            sf::Text tr,tn; if(Fok){tr.setFont(F);tn.setFont(F);}
